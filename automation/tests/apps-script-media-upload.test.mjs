...
test('aceita WebP referenciado e monta cabeçalhos inline', () => {
  ...
  assert.doesNotThrow(() =>
    api.topcValidarUploadMidia_(media, 'image/webp', 1024, ['foto.webp'])
  );
  assert.deepEqual(
    structuredClone(api.topcMontarCabecalhosMidia_(media, 'a'.repeat(64))),
    {
...
test('rejeita SHA-256 inválido', () => {
  const api = loadScripts([corePath]);
  assert.throws(() => api.topcMontarCabecalhosMidia_({
    mediaKey: 'foto.webp', role: 'cover', position: 0
  }, 'abc'));
});

test('assina e envia exatamente os mesmos bytes', () => {
  const bytes = [0, 1, 127, -128, -1];
  let digested;
  let request;
  const api = loadScripts([corePath, uploadPath], {
    TOPC_SITE_AUTOMATION: { BASE_URL: 'https://example.test' },
    ...
    topcBytesParaHex_: value => value.map(b =>
      ((b + 256) % 256).toString(16).padStart(2, '0')
    ).join(''),
    topcHmacSha256Hex_: canonical => {
      assert.equal(
        canonical,
        '1700000000\nreq-1\npauta-1\n' + '01'.repeat(32)
      );
  ...
  api.topcEnviarBytesAssinados_(
    '/api/automation/articles/pauta-1/media',
    'pauta-1',
    bytes,
    'image/webp',
    { 'x-media-key': 'foto.webp' }
  ...
  assert.strictEqual(digested, bytes);
  assert.strictEqual(request.options.payload, bytes);
  assert.equal(request.options.contentType, 'image/webp');
  assert.equal(request.options.headers['X-Automation-Signature'], 'f'.repeat(64));
  assert.equal(request.options.headers['x-media-key'], 'foto.webp');
});
...
test('aceita somente HTTP 200 e 201 no upload', () => {
  ...
  assert.throws(
    () => api.topcValidarRespostaUpload_(400, 'foto.webp', 'x'.repeat(800)),
    error => error.message.includes('HTTP 400') && error.message.length < 600
  );
});
